        ...
        for (int i = 0; i < 10; i++) {
            ...
            for (int t = 0; t < couples.size(); t++) {
                if (left.contains(couples.get(t))) {
                    String add = right.get(left.indexOf(couples.get(t)));
                    int index = polymer.indexOf(add);
                    occurrency.set(index, occurrency.get(index) + count.get(t));
                    String first = couples.get(t).substring(0, 1) + add;
                    String second = add + couples.get(t).substring(1, 2);
